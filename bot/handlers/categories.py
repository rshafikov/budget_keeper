...
async def get_user_categories(message: Message, user: dict):
    await message.answer(
...
async def change_category_welcome(message: Message, state: FSMContext, user: dict):
    await state.clear()
    await message.answer(
...
async def delete_category_welcome(message: Message, state: FSMContext, user: dict):
    await state.clear()
    await message.answer(
...
async def create_category_get_name(message: Message, state: FSMContext, user: dict):
    print(user)
...
async def change_category_get_old_name(message: Message, state: FSMContext, user: dict):
    print(user)
...
async def delete_category_get_name(message: Message, state: FSMContext, user: dict):
    print(user)
    if message.text not in user['categories']:
        await message.answer(
            f'Извините, категория <b>{message.text}</b> не найдена. '
...
async def create_category_callback(
    ...
    if call.data == 'category.create.no':
        await call.message.answer('Создание категории отменено.')
        return
    ...
    if new_category is not None:
        ...
        user = await api_client.user_profile(token)
        user_storage[call.from_user.id] = user
        return

    await call.message.answer('Произошла ошибка во время создания категории. '
